// lib/ImageUtils.js
// Comprehensive image utility solution to fix all image loading issues permanently
...
const ImageUtils = {
  ...
  STABILITY_API_KEY:
    ...
    "sk-trv7dzLw2Zv4RYWxMWxP7Pj9olGUSZfPYFbujrydnbUMA9h7",
  ...
  /**
   * NEW METHOD: Generate an image using Stability AI
   * This is the primary image generation method that will be used for guaranteed image generation
   * @param {string|object} promptInput - The prompt for image generation (can be string or JSON)
   * @param {Object} options - Additional options like size
   * @returns {Promise<Object>} Generated image data
   */
  async generateImageWithStabilityAI(promptInput, options = {}) {
    ...
    if (typeof promptInput === "string") {
      ...
      if (Array.isArray(promptInput)) {
        promptText = promptInput.map((msg) => msg.content || "").join(" ");
        // Handle message object format
      } else if (promptInput.content) {
        promptText = promptInput.content;
        // Handle raw prompt object
      } else if (promptInput.prompt) {
        promptText = promptInput.prompt;
      } else {
        // Stringify the object but limit length
        promptText = JSON.stringify(promptInput).substring(0, 1000);
      }
    } else {
      promptText = String(promptInput).substring(0, 1000);
    }

    // Ensure prompt is not too long for Stability API (max 2000 chars)
    promptText = promptText.trim().substring(0, 1900);

    // Add default descriptors for better image quality
    promptText =
      `${promptText}. High quality, detailed, professional photograph.`.substring(
        0,
        1950
      );

    // Create a deterministic cache key
    const cacheKey = promptText.toLowerCase().trim().substring(0, 100);

    // Check cache first to avoid redundant API calls
    if (imageCache.stabilityai[cacheKey]) {
      console.log("Using cached Stability AI image for:", cacheKey);
      return imageCache.stabilityai[cacheKey];
    }

    try {
      console.log(
        "Generating image with Stability AI for prompt length:",
        promptText.length
      );

      // Call our internal API endpoint that wraps the Stability AI API
      ...
      if (data.error) {
        console.error("Stability AI API returned error:", data.error);
      ...
      // Create image object in the same format as our other sources
      const imageData = {
        url: data.imageBase64,
        alt: `AI generated image for ${promptText.substring(0, 50)}...`,
      ...
      // Cache the result
      imageCache.stabilityai[cacheKey] = imageData;
      ...
      // Try Unsplash next as a first fallback
      try {
        console.log("Falling back to Unsplash...");
        return await this.fetchUnsplashImage(promptText, options);
      } catch (unsplashError) {
        console.error("Unsplash fallback failed:", unsplashError);

        // Try Pexels as a second fallback
        try {
          console.log("Falling back to Pexels...");
          return await this.fetchPexelsImage(promptText, options);
        } catch (pexelsError) {
          console.error("Pexels fallback failed:", pexelsError);

          // Final fallback to our static library
          console.log("Using static image library as final fallback");
   ...
   * Now improved with multiple fallbacks and stability AI integration
  ...
  async getOptimizedImage(category = "business", options = {}) {
    const { width = 800, height = 600, useAPI = true, useAI = true } = options;

    try {
      // If AI generation is enabled, try Stability AI first
      if (useAPI && useAI) {
        try {
          // Create a more descriptive prompt for better results
          const enhancedPrompt = `High quality professional ${category} image. Realistic, detailed, well-lit, 4K.`;
          return await this.generateImageWithStabilityAI(
            enhancedPrompt,
            options
          );
        } catch (aiError) {
          console.error("AI image generation failed:", aiError);
          // Continue to other methods if AI fails
        }
      }

      // If APIs are enabled (but AI failed or wasn't used), try fetching fresh images
      if (useAPI) {
        try {
      ...
      console.error("All external image sources failed:", error);
      // Continue to fallback
    }

    // Use the static library as final fallback
