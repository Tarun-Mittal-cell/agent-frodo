...
const CACHE_DIR = path.join(os.tmpdir(), "Frodo-website-generator");
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          // Continue to other APIs
          this.logger.warn(`Failed to get UI Faces image:`, error.message);
        }
      }

      // Try Lorem Space with category mapping
      try {
        const loremCategory = this._mapCategoryToLoremSpace(category);
        if (PREMIUM_IMAGE_APIS.loremSpace.categories.includes(loremCategory)) {
          const imageUrl =
            PREMIUM_IMAGE_APIS.loremSpace.getDirectUrl(loremCategory);
          this.imageCache.set(cacheKey, imageUrl);
          return imageUrl;
        }
      } catch (error) {
        // Continue to other APIs
        this.logger.warn(
          `Failed to get Lorem Space image for ${category}:`,
          error.message
        );
      }
      ...
      this.logger.error(`Error getting image URL for ${category}:`, error);
      this.errorCounts.imageLoading++;
